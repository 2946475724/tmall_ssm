...
import com.zs.tmall.util.UploadedImageFile;
...
public class ProductImageController {
    ...
    public void imageAddProcess(ProductImage productImage,HttpSession session,UploadedImageFile uploadedImageFile) throws IOException{
        ...
        if (ProductImageService.TYPE_SINGLE.equals(productImage.getType())){
            imageFolder = session.getServletContext().getRealPath("images/productSingle");
            imageFolderSmall = session.getServletContext().getRealPath("images/productSingle_small");
            imageFolderMiddle = session.getServletContext().getRealPath("images/productSingle_middle");
        }else {
            //如果上传的是产品详情图片缩略图
            imageFolder = session.getServletContext().getRealPath("images/productDetail");
        }
